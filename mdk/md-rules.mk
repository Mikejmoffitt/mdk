...
MDKSOURCES_ASM := $(shell find $(MDKSRCDIR)/ -type f -name '*.a68')
...
.PHONY: all vars $(MDK_C2_TESTROM) test_c2 $(EXTERNAL_DEPS)
...
$(OBJDIR)/%.o: $(SRCDIR)/%.a68 $(OBJECTS_RES) $(EXTERNAL_DEPS)
	@mkdir -p $(dir $@)
...
$(OBJDIR)/%.o: $(MDKSRCDIR)/%.a68 $(OBJECTS_RES) $(EXTERNAL_DEPS)
	@mkdir -p $(dir $@)
	@bash -c 'printf " \e[33m[C:ASM]\e[0m $< --> $@\n"'
	awk '{gsub(/;/,";#"); printf("%s", $$0 RT)}' RS='"[^"]*"' $< | awk '{gsub(/\$$/,"0x"); printf("%s", $$0 RT)}' RS='"[^"]*"' | $(AS) $(ASFLAGS) -o $@ -c -

# Converts res.a68 and other intermediate files generated by util
$(OBJDIR)/%.o: $(OBJDIR)/%.a68
	@bash -c 'printf " \e[33m[B:ASM]\e[0m $< --> $@\n"'
	$(AS) $(ASFLAGS) -c $< -o $@

# Converts a file to object files
$(OBJDIR)/res.a68: $(BIN2S) $(EXTERNAL_DEPS)
	mkdir -p $(dir $@)
...
$(MDK_C2_TESTROM): $(OUTPUT_GEN) $(BSPLIT)
	...
	dd if=$(MDK_C2_ADPCM) of=$@/epr-16810.ic4 bs=256 count=2048
...
clean:
	...
	-rm -f $(OUTPUT_ELF)
	-rm -f $(OBJECTS_RES) $(OBJDIR)/res.a68 $(RES_HEADER)
	-rm -rf $(OBJDIR)
